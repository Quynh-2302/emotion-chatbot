...
def network(x, keep=0.8):
    ...
    network = tl.layers.Conv1dLayer(network, act=tf.nn.relu, shape=[
                                    3, 200, 6], name='cnn_layer1', padding='VALID')
    ...
    network = tl.layers.DenseLayer(
        network, n_units=6, act=tf.identity, name="output")
...
if __name__ == '__main__':
    ...
    x = tf.compat.v1.placeholder(
        "float", [None, max_seq_len, 200], name="inputs")
