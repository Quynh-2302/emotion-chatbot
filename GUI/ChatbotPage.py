...
from generation import load_generation_model, gen
import random
import json
import tensorflow as tf
import requests
from generation35 import chat_with_gpt3_5
# from update_emotion_data import update_emotion_data
...
class ChatbotPage(tk.Frame):
    def __init__(self, parent, root):
        ...
        self.sess2 = load_generation_model(
            model_name=self.model_name)    # Start gpt-2 model
    ...
    def create_chat_frame(self):
        ...
        textShow.tag_config("user", foreground="blue",
                            font=self.config["message"]["font"])
        textShow.tag_config("littlegenesis", foreground="green",
                            font=self.config["message"]["font"])
        textShow.tag_config("input_sentence", foreground="green",
                            font=self.config["message"]["font"])
        ...
        def response_strategy(f_reponse, e_response, input_sentence, emotion_threshold):
            if str.lower(input_sentence) in f_reponse:
                ...
                result_ER = requests.post(
                    self.emotion_recognition_url, json=data)
                ...
                sql = "INSERT INTO emotion_info" \
                      ...
                      "VALUES (%d, %f, '%s', '%s','%s')" % \
                      (label, value, current_time,
                       input_sentence, self.root.username)
                ...
                for k in topics.keys():
                    if k in input_sentence:
                        ...
                        answer = gen(self.sess2, prefix,
                                     self.model_name, include_prefix=True) + "."
                        return answer
                ...
                if value >= threshold[label][1]:
                    ...
                    print("prefix: ", prefix)
                    answer = gen(self.sess2, prefix, self.model_name,
                                 include_prefix=True) + "."
                    return answer
                else:
                    print("chatgpt3.5")
                    answer = chat_with_gpt3_5(
                        input_sentence, chat_history) + "."
                    return answer
        ...
        def give_response():
            ...
            textShow.insert(tk.END,
                            response_strategy(
                                fix_response, emotion_response, q, emotion_threshold),
                            "input_sentence")
            textShow.insert(tk.END, "\n\n", "littlegenesis")
            textShow.see(tk.END)
            textShow.config(state='disabled')
